  ...
  if (granted) {
    ...
    const contacts: Contact[] = data.map((item: any) => {
      return {
        ...
        phone: {
          countryCode: "1",
          number: item.phoneNumbers?.[0]?.digits?.startsWith("+1")
            ? item.phoneNumbers[0].digits.slice(2)
...
const mergeContacts = (
  ...
  fetched_contacts: Contact[]
): Contact[] => {
  const mergedContacts: Contact[] = [...old_contacts];

  fetched_contacts.forEach((fetched_contact) => {
    const existingIndex = old_contacts.findIndex(
      (old_contact) => old_contact.id === fetched_contact.id
    );
...
export const syncContactsToHalabi = async (
  dispatch: any,
  old_contacts: Contact[]
): Promise<boolean> => {
  const { granted } = await Contacts.requestPermissionsAsync();
...
export const syncContactsToNative = async (
  ...
  for (const halabiContact of halabiContacts) {
    ...
    const halabiBirthdayDay =
      parseInt(halabiContact.birthday?.day || "", 10) || 0;
    ...
    if (nativeContact) {
      ...
      dispatch(
        updateContactId({ oldId: halabiContact.id, newId: added_contact_id })
      );
    }
  }
  return true;
